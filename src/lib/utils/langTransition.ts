...
import { ANIM_DURATION, AnimEasing, Lang } from '$lib/constants';

export function animateLangSwitch(container: HTMLElement, currentLang: string): void {
	const clipFrom = currentLang === Lang.EN ? 'inset(0 100% 0 0)' : 'inset(0 0 0 100%)';
		...
		if (element.getAttribute('data-lang') === currentLang) {
			...
			requestAnimationFrame(() => {
				animate(
					element,
					{ clipPath: 'inset(0)' },
					{ duration: ANIM_DURATION, ease: easing },
				);
			});
		} else {
